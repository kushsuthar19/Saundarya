...
Two jobs:
  - run_winback_job()            (template #10 'winback', Marketing) —
    any client whose last_visit was exactly 60 days ago.
  - run_membership_expiry_job()  (template #11 'membership', Utility) —
    Exclusive memberships expiring in exactly 7, 3, or 0 days.

Both are best-effort per client: one client's send failing (bad phone,
...
def start_scheduler():
    ...
    if _scheduler.running:
        return
    # 09:00 / 09:05 local server time — staggered a few minutes apart so
    # the two jobs don't compete for DB connections at the exact same tick.
    ...
    _scheduler.start()
    logger.info("WhatsApp reminder scheduler started (winback 09:00, membership expiry 09:05 daily).")
